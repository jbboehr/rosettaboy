...
static inline void gpu_paint_tile_line(struct GPU *self, i16 tile_id,
                                       SDL_Point *offset, SDL_Color *palette,
                                       bool flip_x, bool flip_y, i32 y) {
...
static inline void gpu_paint_tile(struct GPU *self, i16 tile_id,
                                  SDL_Point *offset, SDL_Color *palette,
...
static inline void gpu_draw_line(struct GPU *self, i32 ly) {
    ...
    if (lcdc & LCDC_BG_WIN_ENABLED) {
        ...
        for (int lx = 0; lx <= 160; lx += 8) {
            ...
            gpu_paint_tile_line(self, tile_id, &xy, self->bgp, false, false,
                                tile_sub_y);
        }
    }
    ...
    if (lcdc & LCDC_OBJ_ENABLED) {
        ...
        for (int n = 0; n < 40; n++) {
            ...
            if (sprite_is_live(&sprite)) {
                ...
                // OAM_BASE + (sprite_id * 4) + 0, x, y);
                SDL_Point xy = {
                    .x = sprite.x - 8,
                    .y = sprite.y - 16,
                };
                gpu_paint_tile(self, sprite.tile_id, &xy, palette,
                               sprite.x_flip, sprite.y_flip);

                if (dbl) {
                    xy.y = sprite.y - 8;
                    gpu_paint_tile(self, sprite.tile_id + 1, &xy, palette,
                                   sprite.x_flip, sprite.y_flip);
                }
            }
        }
...
struct GPU gpu_ctor(struct CPU *cpu, struct RAM *ram, char *title,
                    bool headless, bool debug) {
    struct GPU self = {.cpu = cpu,
                       .ram = ram,
                       .debug = debug,
                       .colors = {
    ...
    if (!headless) {
        ...
        self.hw_buffer =
            SDL_CreateTexture(self.hw_renderer, SDL_PIXELFORMAT_ABGR8888,
...
void gpu_tick(struct GPU *self) {
    ...
    if (lx == 0 && ly < 144) {
        ...
        if (ly == 143) {
            ...
            if (self->hw_renderer) {
                SDL_UpdateTexture(self->hw_buffer, NULL, self->buffer->pixels,
                                  self->buffer->pitch);
