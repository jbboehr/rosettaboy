...
NORETURN static void timeout_err(int frames, double duration) {
    fprintf(stdout, "Emulated %5d frames in %5.2fs (%.0ffps)", frames, duration,
            frames / duration);
    fflush(stdout);
    exit(0);
}
...
NORETURN static void invalid_ram_read_err(u8 ram_bank, int offset,
                                          u32 ram_size) {
    fprintf(stdout,
            "Read from RAM bank 0x%02X offset 0x%04X >= ram size 0x%04X",
            ram_bank, offset, ram_size);
    fflush(stdout);
    abort();
}

NORETURN static void invalid_ram_write_err(u8 ram_bank, int offset,
                                           u32 ram_size) {
    fprintf(stdout, "Write to RAM bank 0x%02X offset 0x%04X >= ram size 0x%04X",
            ram_bank, offset, ram_size);
    fflush(stdout);
    abort();
}
