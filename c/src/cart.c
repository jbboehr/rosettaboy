...
struct Cart cart_ctor(const char *filename, bool debug) {
    ...
    self.data = (unsigned char *)mmap(NULL, (size_t)statbuf.st_size, PROT_READ,
                                      MAP_SHARED, fd, 0);

    memcpy(self.logo, self.data + 0x0104, 48);
    memcpy(self.name, self.data + 0x0134, 16);
    self.is_gbc =
        self.data[0x143] == 0x80; // 0x80 = works on both, 0xC0 = colour only
    ...
    if (self.ram_size) {
        ...
        self.ram = (unsigned char *)mmap(NULL, (size_t)self.ram_size,
